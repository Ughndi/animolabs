...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateSelectedSeats() {
        const hasTakenSeat = selectedSeats.some(seatNumber => {
            const seat = seats.find(seat => seat.seatNumber === seatNumber);
            return seat.status === "taken";
        });
    
        if (selectedSeats.length > 0 && !hasTakenSeat) {
    ...
    function isSeatTaken(selectedSeat) {
        // Find the corresponding seat object
        const seat = seats.find(seat => seat.seatNumber === selectedSeat);
        // Return true if the seat status is "taken", otherwise return false
        return seat && seat.status === "taken";
    }
